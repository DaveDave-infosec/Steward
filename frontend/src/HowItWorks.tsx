  ...
  return (
    <div className="howto">
      ...
      <section className="howto-block">
        ...
        <p><strong>Settlement is bound to the verdict.</strong> Settlement is not driven by caller-supplied data. apply_verdict is given only a case id; the reserve reads that verdict directly from the verifier contract on-chain, requires it to have been produced against this checkpoint's exact locked evidence source and complete criteria, authenticates the transaction sender, accepts only the most recent verdict for that checkpoint, and moves capital strictly by the verifier's stored fulfilment percentage and outcome. A relayer cannot fake the number, spoof an identity, point at a different repository, or submit an older, more favourable verdict. What remains possible is repetition rather than selection: a creator can run a review again, but every attempt is recorded on-chain and only the newest one can settle. A relaying transaction still triggers the read today; a fully event-driven trigger is a later refinement.</p>
